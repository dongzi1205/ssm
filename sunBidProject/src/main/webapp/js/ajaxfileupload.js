jQuery
		.extend({
			...
			//handleError这个函数是自己加的，因为jquery低版本有这个函数，选择高的版本没有了，所以要加，否则出错
			...
			uploadHttpData : function(r, type) {
				...
				if (type == "json") {
					...
					// The pre tag in Chrome has attribute, so have to use regex
					// to remove
					var data = r.responseText;
					var rx = new RegExp("<pre.*?>(.*?)</pre>", "i");
					var am = rx.exec(data);
					// this is the desired data extracted
					var data = (am) ? am[1] : ""; // the only submatch or
					// empty
					eval("data = " + data);
				}
